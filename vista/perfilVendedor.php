    ...
    <div class="profile-card">
        ...
        <div class="stats-row">
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="stat-item">
                        <div class="stat-number"><?= $pedidos_count ?></div>
                        <div class="stat-label">Pedidos</div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="stat-item">
                        <div class="stat-number"><?= htmlspecialchars($vendedor['categoria']) ?></div>
        ...
        <div class="profile-body">
            ...
            <div class="row mt-4">
                ...
                <div class="col-md-4 mb-3">
                    <a href="cambiarPassword.php" class="btn btn-outline-warning w-100">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('perfilForm').addEventListener('submit', function(e) {
        ...
        const guardarIcon = document.getElementById('guardarIcon');
        ...
        .then(data => {
            if (data.success) {
                ...
                // Update page data if returned
                if (data.data && data.data.vendedor) {
                    updatePageData(data.data.vendedor);
                }
        ...
        .finally(() => {
            ...
            guardarIcon.style.display = 'inline';
        });
    });
    ...
    function updatePageData(vendedor) {
        // Update the displayed information sections
        const infoSections = document.querySelectorAll('.info-card p');
        
        // Update empresa info display
        const nombreDisplay = Array.from(infoSections).find(p => p.innerHTML.includes('Nombre:'));
        if (nombreDisplay) {
            nombreDisplay.innerHTML = `<strong>Nombre:</strong> ${escapeHtml(vendedor.nombre_empresa)}`;
        }
        
        const categoriaDisplay = Array.from(infoSections).find(p => p.innerHTML.includes('Categoría:'));
        if (categoriaDisplay) {
            categoriaDisplay.innerHTML = `<strong>Categoría:</strong> ${escapeHtml(vendedor.categoria)}`;
        }
        
        const telefonoDisplay = Array.from(infoSections).find(p => p.innerHTML.includes('Teléfono:'));
        if (telefonoDisplay) {
            telefonoDisplay.innerHTML = `<strong>Teléfono:</strong> ${escapeHtml(vendedor.telefono)}`;
        }
