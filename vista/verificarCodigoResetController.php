    ...
    switch ($action) {
        ...
        case 'reset_password':
            ...
            if ($is_pdo) {
                ...
                $stmt->execute([$correo, $_SESSION['codigo_verificado']]);
                $row = $stmt->fetch(PDO::FETCH_ASSOC);
            } else {
                $stmt = $conn->prepare("SELECT id FROM clientes WHERE correo = ? AND codigo_verificacion = ?");
                $stmt->bind_param("ss", $correo, $_SESSION['codigo_verificado']);
                $stmt->execute();
                $resultado = $stmt->get_result();
                $row = $resultado->fetch_assoc();
            }
            ...
            if ($row) {
                ...
                if ($is_pdo) {
                    ...
                    $update_result = $stmt_update->execute([$nueva_contrasena_hash, $cliente_id]);
                } else {
                    $stmt_update = $conn->prepare("UPDATE clientes SET contrasena = ?, codigo_verificacion = NULL WHERE id = ?");
