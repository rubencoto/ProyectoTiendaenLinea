...
try {
    ...
    // Procesar dirección de envío después de obtener info del cliente
    $direccion_envio_info = null;
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['direccion_seleccionada'])) {
        $direccion_seleccionada = $_POST['direccion_seleccionada'];

        if ($direccion_seleccionada === 'nueva') {
            // Validar campos requeridos para nueva dirección
            if (empty($_POST['nueva_direccion_completa']) || empty($_POST['nueva_provincia'])) {
                throw new Exception("Datos de dirección incompletos");
            }

            $nueva_direccion = [
                'direccion_completa' => $_POST['nueva_direccion_completa'],
                'provincia' => $_POST['nueva_provincia'],
                'canton' => $_POST['nueva_canton'] ?? '',
                'distrito' => $_POST['nueva_distrito'] ?? '',
                'alias' => $_POST['nueva_alias'] ?? 'Dirección temporal'
            ];

            // Si se marcó guardar dirección, guardarla
            if (!empty($_POST['guardar_direccion'])) {
                $datos_direccion = [
                    'cliente_id' => $cliente_id,
                    'etiqueta' => $nueva_direccion['alias'],
                    'nombre' => $cliente['nombre'],
                    'apellidos' => $cliente['apellido'],
                    'telefono' => $cliente['telefono'],
                    'codigo_postal' => '',
                    'linea1' => $nueva_direccion['direccion_completa'],
                    'linea2' => '',
                    'provincia' => $nueva_direccion['provincia'],
                    'canton' => $nueva_direccion['canton'],
                    'distrito' => $nueva_direccion['distrito'],
                    'referencia' => '',
                    'is_default' => false
                ];

                $direccion_guardada = $direccionesManager->agregarDireccionCliente($datos_direccion);

                if ($direccion_guardada) {
                    error_log("Nueva dirección guardada para cliente: $cliente_id");
                }
            }

            $direccion_envio_info = $nueva_direccion;

        } elseif ($direccion_seleccionada === 'perfil') {
            // Usar dirección del perfil principal
            $direccion_envio_info = [
                'direccion_completa' => $cliente['direccion'] ?? 'Dirección no especificada',
                'provincia' => '',
                'canton' => '',
                'distrito' => '',
                'alias' => 'Dirección del perfil'
            ];

        } else {
            // Usar dirección guardada seleccionada
            $direccion_guardada = $direccionesManager->obtenerDireccionPorId($direccion_seleccionada, $cliente_id);
            if ($direccion_guardada) {
                $direccion_envio_info = [
                    'direccion_completa' => $direccion_guardada['linea1'],
                    'provincia' => $direccion_guardada['provincia'] ?? '',
                    'canton' => $direccion_guardada['canton'] ?? '',
                    'distrito' => $direccion_guardada['distrito'] ?? '',
                    'alias' => $direccion_guardada['etiqueta'] ?? 'Dirección guardada'
                ];
            } else {
    ...
    do {
        $stmt_check = $conn->prepare("SELECT id FROM pedidos WHERE numero_orden = ?");
    ...
    try {
        // Insertar orden en la tabla pedidos
        $stmt_orden = $pdo_conn->prepare("
            INSERT INTO pedidos (numero_orden, cliente_id, subtotal, envio, total, estado, fecha_orden)
        ...
        foreach ($productos_comprados as $producto) {
            $stmt_detalle->execute([$orden_id, $producto['id'], $producto['cantidad'], $producto['precio'], $producto['subtotal']]);
            error_log("ConfirmarOrden: Inserted detail for order_id: $orden_id, product_id: " . $producto['id'] . ", cantidad: " . $producto['cantidad']);
        }
        ...
        foreach ($productos_comprados as $producto) {
            ...
            $stmt_check_stock->execute([$producto['id']]);
            $current_stock = $stmt_check_stock->fetchColumn();

            if ($current_stock < $producto['cantidad']) {
                throw new Exception("Stock insuficiente para el producto ID: " . $producto['id'] . ". Stock disponible: $current_stock, cantidad solicitada: " . $producto['cantidad']);
            }

            // Actualizar stock solo si hay suficiente
            $updated = $stmt_stock->execute([$producto['cantidad'], $producto['id'], $producto['cantidad']]);
            if (!$updated || $stmt_stock->rowCount() === 0) {
                throw new Exception("Error al actualizar stock para producto ID: " . $producto['id']);
            }
    ...
    // Construir texto de dirección para los correos
    $direccion_texto = '';
    if ($direccion_envio_info) {
        $direccion_texto = "
        <h3>Dirección de Envío</h3>
        <p><strong>Dirección:</strong> {$direccion_envio_info['direccion_completa']}</p>";

        if (!empty($direccion_envio_info['provincia'])) {
            $direccion_texto .= "<p><strong>Ubicación:</strong> {$direccion_envio_info['provincia']}";
            if (!empty($direccion_envio_info['canton'])) {
                $direccion_texto .= ", {$direccion_envio_info['canton']}";
            }
            if (!empty($direccion_envio_info['distrito'])) {
                $direccion_texto .= ", {$direccion_envio_info['distrito']}";
            }
            $direccion_texto .= "</p>";
        }
    }
    ...
    if ($direccion_envio_info) {
        echo '
            <div class="direccion-info">
                ...
                <p><strong>' . htmlspecialchars($direccion_envio_info['direccion_completa']) . '</strong></p>';

        if (!empty($direccion_envio_info['provincia'])) {
            echo '<p><small>' . htmlspecialchars($direccion_envio_info['provincia']);
            if (!empty($direccion_envio_info['canton'])) {
                echo ', ' . htmlspecialchars($direccion_envio_info['canton']);
            }
            if (!empty($direccion_envio_info['distrito'])) {
                echo ', ' . htmlspecialchars($direccion_envio_info['distrito']);
            }
            echo '</small></p>';
        }
        echo '</div>';
